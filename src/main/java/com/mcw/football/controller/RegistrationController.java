...
public class RegistrationController {
    ...
    public String addUser(@RequestParam("passwordConfirm") String passwordConfirm,
        ...
        if (isConfirmEmpty) {
            bindingResult.addError(new FieldError(user.getUsername(),"passwordConfirmError", "Password confirm can't be empty"));
        }
        if (user.getPassword() != null && !user.getPassword().equals(passwordConfirm)) {
            bindingResult.addError(new FieldError(user.getUsername(),"passwordError", "Passwords are not equal"));
        }
        if(Strings.isNullOrEmpty(user.getFullName())){
            bindingResult.addError(new FieldError(user.getUsername(),"fullNameError", "Full Name must be not empty"));
        }
